...
for  j=2:n-1
    for i=2:n-1
     ...
     u(j,1)= (1/((gamma*h^2)-4))*((h^2)*F(i,j)-(u(i+1,j)+u(i+1,j)+u(i,j+1)+u(i,j-1))); %This is the boundary conditions. %This is the boundary conditions.
     %u(i,j)=(1/((gamma*h^2)-4))*((h^2)*F(i,j)-(u(i+1,j)+u(i-1,j)+2*u(i,j+1)));
end
unew=u;
